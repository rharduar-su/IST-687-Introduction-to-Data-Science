...
svm <- data.frame(Score=as.factor(airData$Likelihood.to.recommend), Age= as.factor(airData$Age), Price = as.factor(airData$Price.Sensitivity),
                 ArrivalDelay= as.factor(airData$Arrival.Delay.in.Minutes), DepartDelay=as.factor(airData$Departure.Delay.in.Minutes),FlightsPerYear=as.factor(airData$Flights.Per.Year),
                 TypeofTravel = as.factor(airData$Type.of.Travel), Class= as.factor(airData$Class), Shopping = as.factor(airData$Shopping.Amount.at.Airport),
                 Eating = as.factor(airData$Eating.and.Drinking.at.Airport),PartnerName = as.factor(airData$Partner.Name),
               Status = as.factor(airData$Airline.Status), Cancelled = as.factor(airData$Flight.cancelled))

str(svm)
airData$Likelihood.to.recommend[is.na(airData$Likelihood.to.recommend)]=na.rm=TRUE

randIndex <- sample(1:dim(svm)[1])
cutPoint2_3 <-floor(2*dim(svm)[1]/3)
trainData <- svm[randIndex[1:cutPoint2_3],]
testData <- svm[randIndex[(cutPoint2_3+1):dim(svm)[1]],]

class(trainData)
svmOutput <- ksvm(Score ~., data=trainData, kernel="rbfdot", kpar = "automatic",C=5, cross=3,prob.model=TRUE)
...
table(compTable)
str(compTable)
sum(diag(table(svmPred, testSet$Score)))/sum(table(svmPred, testSet$Score)
cmatrix <- confusionMatrix(svmPred, testSet$Score)
